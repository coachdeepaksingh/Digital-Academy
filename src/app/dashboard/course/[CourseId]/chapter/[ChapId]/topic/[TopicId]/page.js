...
export default function CreateContent({params}) {

    const router = useRouter();
    // const schema = yup.object({
    //       topContent:yup.string().required('Content is required.'),
    //       urlPath:yup.string().required('URL is required.'),
    //   });
    ...
    const onSubmit = async (data) => {
      await axios.post('http://localhost:3000/api/Contents/AddContent', data)
      .then((response) => {
    ...
    useEffect(function () {
        axios.get("http://localhost:3000/api/Topics/GetAllTopics?chapId=" + params.ChapId).then((response)=>
            setTop(response.data)).then((err)=>console.log(err));
      },[params.ChapId]);

    //getting the list of chapters...
    useEffect(function () {
      axios.get("http://localhost:3000/api/Contents/GetAllChaptersAndTopics?courseId="+params.CourseId+"&chapId=" + params.ChapId).then((response)=>
          setCont(response.data)).then((err)=>console.log(err));
